...
export default function CoursePreviewPanel() {
  ...
  // styleAsPersist: true if we want to style as "persist" (not "temp")
  const styleAsPersist = 
    !tempCourse ||
    (persistCourse !== null && tempCourse?.id === persistCourse.id);
    ...
    if (!loading) {
      ...
      return (
        <Skeleton 
          style={{
            position: 'fixed',
            ...mapPositionToCoords(incomingPos),
            width: "47.5%",
            backgroundColor: "white",
            padding: "20px",
            borderRadius: "12px",
            pointerEvents: 'auto',
            boxShadow: styleAsPersist
              ? '0 0 0 4px rgba(209, 99, 145, 0.25), 0 6px 18px rgba(0, 0, 0, 0.1)'
              : 'none',
            border: styleAsPersist
              ? '1px solid #811331'
              : '1px solid #ccc',
