...
const BAR_GRADES = ["S", "P", "N", "W", "NG"];

export const BarChart = ({ distribution, isMobile = true }: { distribution: Distribution, isMobile: boolean }) => {
  ...
  const filteredGrades = Object.entries(grades)
    ...
    .map(([grade, count]) => [grade, count as number] as [string, number])
    .sort(
      ([gradeA], [gradeB]) =>
  ...
  const maxValue = Math.max(...Object.values(grades).map(Number));

  const totalStudents: number = Object.values(grades).reduce(
    (total: number, gradeCount: any) => total + gradeCount,
    0
  );
  ...
  const barHeights = filteredGrades.map(([grade, count]: [string, number]) => ([
    grade,
    count,
    ...
    >
      {
        ...
        barHeights.map(([grade, , height, color], i) => (
          <g key={grade}>
            ...
            <rect
              x={i * barWidth}
              y={BAR_GRAPH_HEIGHT - (height as number)}
              width={barWidth}
              height={height}
              style={{
